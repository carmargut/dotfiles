...
# Display local weather collected using darksky-weather (install from brew)
...
print_temperature() {
    ...
    echo -n "$(echo $temperature | bc -l | xargs printf "%.0f")"
    echo -n "°$unit_string"
}
...
echo "$(print_weather_icon "$NOW_ICON") $(print_temperature "$NOW_TEMP" "$UNITS") | color='white'"
echo "---"
